...
int main(int argc, char **argv)
{
   ...
   if (!strcmp(argv[4],"put") || !strcmp(argv[4],"get"))
   {
      ...
      char amsg_type = 114;
      short int amsg_len = 71;

      void* ka = malloc(amsg_len);
      memcpy(ka, &amsg_type, sizeof(amsg_type));    //type message
      memcpy(ka+sizeof(amsg_type), &amsg_len, sizeof(amsg_len)); //taille message
      memcpy(ka+sizeof(amsg_type)+sizeof(amsg_len), &rhash_type, sizeof(rhash_type)); //type hash
      memcpy(ka+sizeof(amsg_type)+sizeof(amsg_len)+sizeof(rhash_type), &rhash_len, sizeof(rhash_len));   //taille hash
      memcpy(ka+sizeof(amsg_type)+sizeof(amsg_len)+sizeof(hash_type)+sizeof(hash_len), rhash, sizeof(rhash)); //hash
      printf("%u\n", rhash_type);
      printf("%s\n", rhash);
      ...
      while (1)
      {
         ...
         if (sendto(sockfd, ka, amsg_len, 0, (struct sockaddr *) &dest, addrlen) == -1)
         {
            perror("send to fail \n");
            close(sockfd);
            exit(EXIT_FAILURE);
