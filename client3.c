...
int main(int argc, char **argv)
{
    ...
    if (strcmp(argv[4], "get") == 0)
    {
            ...
            //printf("ipv6: %s \n",str);
             //printf("110 - %d - 5064 - %s - 5518 - %s - addresse\n",taille_final_moins_un,argv[5],argv[3]);
               snprintf(buf, taille_final, "112%d5064%s5518%s%s", taille_final_moins_un, argv[5], argv[3],str);
     ...
     // Ecoute d'un message du tracker
        ...
        // reception de la chaine de caracteres
        		...
        		sscanf(buf2,"%3s%3s%2s%2s%9s%2s%2s%5s%128s",msg_type,msg_length,hash_type,hash_length,hash,client_type,client_size,client_port,client_addresse);

                if (strcmp(msg_type,"111")) // MSG ACK PUT
                {
                    char bufack[2048];

                    snprintf(bufack,2048,"put %s to %s port %s \n",hash,client_addresse,client_port);
                    printf("%s ",bufack);
                }


                if (strcmp(msg_type,"113")) // MSG ACK GET
                {
                    char bufack[2048];
                    char bufack2[2048];

                    snprintf(bufack,2048,"get %s from %s port %s \n",hash,client_addresse,client_port);
                    printf("%s ",bufack);

                    snprintf(bufack2,2048,"IP : %s port %s \n",client_addresse,client_port);
                    printf("%s ",bufack);
                }
